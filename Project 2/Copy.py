...
def print_hangman(hidden_word, num_guesses):
    """Print the gallows, the man, and the blanked-out secret."""
    incorrect_guesses = [guess for letter in num_guesses if not letter in hidden_word]
    word_if_correct_or_incorrect = ' '.join(letter if letter in num_guesses else '_' for letter in hidden_word)
    print(parts_of_the_body[len(incorrect_guesses)])
    print(word_if_correct_or_incorrect)
    print()


def guess(hidden_word, num_guesses):
...
def won(hidden_word, num_guesses):
...
def hanged(hidden_word, num_guesses):
    """Check whether too many guesses have been made."""
    incorrect_guesses = [guess for letter in num_guesses if not letter in hidden_word]
    return len(incorrect_guesses) >= len(parts_of_the_body)
...
def play_hangman():
    ...
    num_guesses = []
    while not hanged(hidden_word, num_guesses):
        print_hangman(hidden_word, num_guesses)
        new_guess = guess(hidden_word, num_guesses)
        if won(hidden_word, num_guesses):
            print('\nWinner!\n')
            return True
    print("\nLoser! The word was {}.\n" .format(hidden_word))
    return False
